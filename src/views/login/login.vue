  ...
  <!--<div class="content">-->
    <!--<div class="wrapper fadeInDown">-->
      <!--<div id="formContent">-->
        ...
        <!--</div>-->
        <!--<input type="text" v-model="username" class="fadeIn second" placeholder="username">-->
        <!--<input type="password" v-model="password" class="fadeIn third"  placeholder="password" @keyup.enter="login">-->
  ...
  <div>
    ...
    <center>
      <el-card class="box-card">
        ...
        <span style="float: left;font-size: 14px;color: #727379">Username</span>
        <el-input
        ...
        <span style="float: left;font-size: 14px;color: #727379;padding-top: 25px">Password</span>
        <el-input
          :type="pass"
          style="padding-top: 5px"
          size="medium"
        ...
        <div style="padding-top: 50px">
          <el-button type="primary" style="width: 220px" @click="login">ĐĂNG NHẬP</el-button>
        </div>
      ...
      <div class="box-card">
        <div style="float: right">
          <el-row>
            <el-button type="text">Điều khoản</el-button>
            <el-button type="text">Bảo mật</el-button>
          </el-row>
        </div>
      </div>
    </center>
...
<script>
  ...
  // import { userInfor } from '@/api/department'
  export default {
    name: 'login',
    data() {
      return {
    ...
    methods: {
      ...
      login() {
        if (this.username && this.password) {
          ...
          this.$http.post(process.env.TEST_LOCAL + '/login', newHotline)
            .then(function(response) {
              if (response.body.status === true && response.body.token) {
                setToken(response.body.token)
                this.$store.state.user.token = getToken()
                // userInfor().then(res => {
                //   console.log(res)
                //   this.$store.state.user.innings = res.data.innings
                // })
